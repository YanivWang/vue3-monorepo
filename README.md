...
| **可观测**                | 双端 `WebMonitor.init`（`@vue3-monorepo/shared/web-monitor`）：`main.ts` 从 Vite 环境装配 URL / debug 等并入 init；附加上报使用 `setAdditionalClientErrorListener`（`ClientErrorPayload`）                                                                                                                                                                                                                                                                                                                                            |
...
| 错误与性能可观测 | 双端 **`WebMonitor.init`**（Core Web Vitals + 全局错误），实现见 `packages/shared/src/web-monitor`；各应用 **`src/main.ts`** 将 `VITE_*` 等装配后传入 init，ingest 接入方式 PC/H5 一致 |
