...
| **Bridge 协议专档**       | H5 宿主（如 App WebView）联调必备：[bridge-protocol.md](apps/h5/h5-template/docs/bridge-protocol.md)，与 **`@vue3-monorepo/js-bridge`** 抽象一致                                                                                                                                                                                                                                                                                                                                                                                        |
| **工程规范**              | ESLint / Stylelint / Prettier + Husky + **lint-staged** + **Commitlint** + TS 类型校验，提交与暂存区双重约束                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **测试与全量门禁**        | 根仓库 **Vitest** 单测（**覆盖率阈值已开**，防退化）；`pnpm run verify:full` 串联引用检查、`check:request-core`（**`@vue3-monorepo/request-core`**）、`check:workspace`（门禁静默跳过防护）、`check:audit`（依赖安全公告棘轮）、`check:theme`（主题产物同步）、类型、Lint、样式、Prettier、测试与 **admin / h5 / docs** 构建。**CI 在每个 PR 上跑同一条命令**                                                                                                                                                                           |
...
| **数据可视化**            | 内置 **ECharts**（`echarts/core` 按需注册，见 `packages/shared/src/utils/echarts.ts`）与 `useECharts` composable，与 PC 模板能力配套                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| 依赖    | 版本                                                                                          |
| ------- | --------------------------------------------------------------------------------------------- |
| Node.js | ≥ 22.23.2（Maintenance LTS，支持到 2027-04；版本以 `.nvmrc` 为准，CI 与 Dockerfile 读同一份） |
...
│   │   │   ├── constants/        # 常量
│   │   │   └── enums/            # 枚举（含自 @vue3-monorepo/js-bridge 再导出的 H5Host）
│   │   ├── package.json
│   │   ├── tsconfig.json
│   │   └── vitest.config.ts
│   ├── request-core/     # @vue3-monorepo/request-core — HTTP 客户端核心（无 UI）
│   ├── web-monitor/      # @vue3-monorepo/web-monitor — Web Vitals 与客户端错误上报
│   └── js-bridge/        # @vue3-monorepo/js-bridge — JSBridge 抽象与多端策略
├── docs/                 # VitePress 项目文档
