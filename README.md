...
| **测试与全量门禁**        | 根仓库 **Vitest** 单测（**覆盖率阈值已开**，防退化）；`pnpm run verify:full` 串联引用检查、`check:request-core`（**`@vue3-monorepo/request-core`**）、`check:workspace`（门禁静默跳过防护）、`check:theme`（主题产物同步）、类型、Lint、样式、Prettier、测试与 **admin / h5 / docs** 构建。**CI 在每个 PR 上跑同一条命令**                                                                                                                                                                                                              |
