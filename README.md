...
| `pnpm`、filter、新代码/依赖放哪 | [Monorepo 现代化工程管理方案](docs/guide/monorepo-workflow.md)、[项目与目录约定](docs/guide/project-conventions.md) |
| 新建 PC/H5 业务应用 | 根 `pnpm run create-app`，详见 [脚手架一键新增业务应用](docs/guide/adding-a-new-app.md) |
| `verify:full`、lint、test、构建 | [代码质量与规范约束](docs/guide/quality-gates.md) |
...
发版/合并前可在本地执行 `pnpm run verify:full`（见 [代码质量与规范约束](docs/guide/quality-gates.md)）。
...
| 新建 PC/H5 业务应用 | `pnpm run create-app`（见 [脚手架一键新增业务应用](docs/guide/adding-a-new-app.md)） |
...
**完整命令表与使用场景**见 [代码质量与规范约束](docs/guide/quality-gates.md)。**Husky、提交信息、PR** 见 [CONTRIBUTING.md](CONTRIBUTING.md) 与 [贡献与协作](docs/guide/contributing.md)。

## 常见问题（摘录）

- **Node / pnpm 版本不符**：见上文 [环境要求](#环境要求)；仍异常见 [排障与 FAQ](docs/guide/troubleshooting.md)。
- **只支持 pnpm**：勿使用 `npm install` / `yarn`。
- **脚本与 CI 在跑什么**：[代码质量与规范约束](docs/guide/quality-gates.md)、[CI 与自动化](docs/guide/ci-and-automation.md)。
