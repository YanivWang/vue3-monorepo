...
- **PC 管理端**：`main.ts` 在 `createApp` 后调用 `WebMonitor.init`（`@vue3-monorepo/web-monitor`，展开 `webMonitorEnvFromVite()` 与 `afterVueError` 等）。如需附加上报消费，使用 **`@vue3-monorepo/web-monitor`** 导出的 `setAdditionalClientErrorListener`（载荷为 `ClientErrorPayload`）。
