...
| `pnpm run lint`                                                | 全仓 ESLint，带 `--max-warnings 0`：**warning 也会让门禁失败**（见下方说明）                                                                                                                                                                                   |
...
| `pnpm run check:workspace`    | `scripts/check-workspace-scripts.mjs`：堵「门禁静默跳过」的三个洞——① 根 `typecheck` 带 `--if-present`，新包漏写 `typecheck` 脚本会被**无声跳过**；② 根 `vitest.config.ts` 的 `test.projects` 是显式清单，不在其中的包写了测试也**不会被 `pnpm test` 发现**；③ 各包主 tsconfig 排除了 `*.spec.ts`，缺 `tsconfig.vitest.json`（或 `typecheck` 脚本没跑它）时测试代码**一行类型都不检查**。三者的失败表现都是「通过」，本脚本把它们变成显式失败 |
...
| `pnpm run check:boundaries`   | `scripts/check-import-boundaries.mjs`：给 §4.1 的边界规则写的**常驻反例**。造 14 个应当被拦的 import，确认每条都报出预期规则后删除。`import-x/no-restricted-paths` 只对**能解析到文件**的 import 生效，缺 resolver 时全部 zone 静默失效——这道门禁就是防那一种                                                                                                                                                                                |
...
>
> **根 `lint` 带 `--max-warnings 0`。** `no-console` 之类的规则配的是 `warn`，而 `eslint` 只在**有 error 时**才以非零码退出——不加这个参数，warning 在 `verify:full` 与 CI 里永远不会让流水线变红，等于没配。各 app 自己的 `lint` 脚本一直带着它，但 `verify:full` 跑的是**根**那条，于是严格的那份从来没被执行过。接入时全仓 warning 数为 0，收紧零成本。
>
