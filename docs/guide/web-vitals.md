...
本页说明 **Web Vitals**（尤其 **Core Web Vitals**）的含义，以及 **PC 管理端与 H5 模板**如何在 **`src/main.ts`** 装配并调用 `@vue3-monorepo/web-monitor`。构建分包与首屏优化见 [Vite 构建优化](./performance.md)；错误与可观测整体见 [全局错误监控](./errors-and-observability.md)。
...
单条指标会包含 `name`、`value`、`rating`、`delta`、`id`、`navigationType`、`page`（pathname + search）、`ts`、`appVersion`、`mode` 等字段，便于接入自建观测或日志管道。字段定义以 **`@vue3-monorepo/web-monitor`** 包内 `packages/web-monitor/src/webVitalsMonitoring.ts` 中序列化逻辑为准。
