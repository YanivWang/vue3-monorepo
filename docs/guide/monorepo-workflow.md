...
- **apps/**：可运行的前端应用，如 `@vue3-mono/admin`、`@vue3-mono/h5`。
- **packages/**：库代码，如 `@vue3-mono/shared`。
- **docs/**：VitePress 包 `@vue3-mono/docs`。

工作区根列表见 [`pnpm-workspace.yaml`](https://github.com/your-org/vue3-monorepo-template/blob/main/pnpm-workspace.yaml)。
...
| **pnpm 过滤单包**                        | `pnpm --filter @vue3-mono/admin <script>`  | 与根脚本等价，CI 里有时直接写这个 |
...
```bash
# 仅给 PC 管理端加运行时依赖
pnpm add <包名> --filter @vue3-mono/admin

# 仅给 H5
pnpm add <包名> --filter @vue3-mono/h5

# 给 shared 加（两端都能 workspace 引用）
pnpm add <包名> --filter @vue3-mono/shared
```

加**开发依赖**（如仅某 app 的插件）时加 `-D`：

```bash
pnpm add -D <包名> --filter @vue3-mono/admin
```
...
| **PC 和 H5 都要** 用：纯类型、常量、与 UI 无关的工具、`request-core` 扩展、通用 hooks 等 | 放 `packages/shared/src`，经 `@vue3-mono/shared/...` 子路径 **export 出去**。             |
...
打开 [`packages/shared/package.json`](https://github.com/your-org/vue3-monorepo-template/blob/main/packages/shared/package.json) 的 **`exports` 字段**。

示例（含义：业务里写 `@vue3-mono/shared/utils` 会解析到 `src/utils/index.ts`）：

```text
@vue3-mono/shared/utils
@vue3-mono/shared/request-core
@vue3-mono/shared/hooks-pc
```
...
更多故障场景见根 [README 故障参考](https://github.com/your-org/vue3-monorepo-template#故障参考) 与 [质量门禁与脚本](./quality-gates.md)。
