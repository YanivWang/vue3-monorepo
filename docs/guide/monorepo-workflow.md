...
上述边界**有机器约束**，不是口头约定：根 `eslint.config.mjs` 的 `import-x/no-restricted-paths` 与 `no-restricted-imports` 会让越界 import 直接 lint 报错，`scripts/check-request-core.js` 再对内核做一次关键字扫描。清单见 [代码质量与规范约束 §4.1](./quality-gates.md)。
