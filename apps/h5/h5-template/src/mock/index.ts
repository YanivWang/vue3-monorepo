 ...
 * H5 运行时 Mock 启动：
 *
 * - 本应用 `mock/**` 下的文件由 vite-plugin-fake-server 在 **dev server 层** 拦截请求，
 ...
 *
 * 当前策略：
 *   · 浏览器  + DEV + VITE_USE_MOCK=true → vite-plugin-fake-server 自动生效，无需额外动作
...
export async function startMock(): Promise<void> {
  ...
  console.info('[mock] Vite dev server 已启用 vite-plugin-fake-server')
}
