...
> 与 iOS / Android 客户端共维。`@vue3-monorepo/js-bridge` 的 `native-app` 策略依赖客户端注入的全局对象与 `postMessage` 约定；以下为业务对齐用的**最小协议说明**，详细字段以各端 SDK 为准。
...
- 协议变更需同步更新本文档版本号与 `@vue3-monorepo/js-bridge` 类型定义。
